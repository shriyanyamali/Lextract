...
To assess the quality of Lextract's output, a pattern-based validity evaluation was conducted, a companion script included in the repository. Fifty market definitions were manually verified from source PDFs and compiled into a reference set. The script analyzes the structural and linguistic patterns of these reference definitions, those being the expected word count range, frequently of certain phrases, and markers characteristic of EC market definitions conclusions, and then scores every definition in the full output against those learned patterns. A definition is considered valid if it satisfies three independent checks: its length falls within the range observed in the reference set, it contains at least one legal phrase appearing in 15% or more of reference definitions, and it contains at least one structural marker typical of EC scope determinations (e.g., relevant market, left open, Commission considers, for the purposes of this decision, etc). Vocabulary overlap was deliberately excluded when analyzing validity for the reference set covers only a narrow slice of industries while the full dataset spans dozens of sectors; penalizing domain diversity would produce false negatives for legitimately extracted definitions in less common industries.
...
![Validity Rates of the Pipeline](images/Lextract_Validity_By_Year.png){ width=90% }
